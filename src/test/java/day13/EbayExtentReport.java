...
public class EbayExtentReport {
	...
	public static void invokeBrowser() {
		oTest = oReport.createTest("Invoke Browser", "User can invoke prefered browser based on input.");
		oTest.assignAuthor("Sathya");
	...
	public static void getPageInfo() throws Exception {
		...
		if(title.equalsIgnoreCase("ebay")) {
			...
			String imagePath = ScreenShotWrapper.takeScreenshotAsFileWithDynamicPath(driver, "EbayTitle");
	...
	public static void searchProduct(String searchTxt,String searchCat) {
		...
		oBtn.click();
		oTest.info("User clicked on searchh button");
	}
	...
	public static void getSearchResult() throws Exception {
		...
		if(iResult > 0) {
			...
			String imagePath = ScreenShotWrapper.takeScreenshotAsFileWithDynamicPath(driver, "ResultCount");
			oTest.fail("Search result is zero.", MediaEntityBuilder.createScreenCaptureFromPath(imagePath).build());
